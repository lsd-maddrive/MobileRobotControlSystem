...
void motor_init()
{
    enum
    {
        ...
        PWM_FREQUENCY = 16000,
        PRESCALER = 1,
        PWM_PERIOD = CLOCK_FREQUENCY/( PWM_FREQUENCY * PRESCALER) - 1  // max 32767
    };
    P1TCONbits.PTCKPS = 0x00;   // PWM time base input clock period is TCY (1:1 prescale)
    P1TCONbits.PTMOD = 0x00;    // PWM time base operates in Free Running mode
